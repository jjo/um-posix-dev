/* $Id: hilos-2-sem.c,v 1.3 2006/04/24 21:03:07 jjo Exp $ */
/*
...
void * hilo(void *arg)
{
	...
	for (i=0;i<hilo_arg->n_iter;i++) {
		if (do_sync)  sem_wait(&hilo_arg->var->semaforo); 
		...
		if (do_sync)  sem_post(&hilo_arg->var->semaforo); 
	}
...
int main(int argc, char **argv) 
{
	...
	mivar.entero=0;
	if (do_sync) sem_init(&mivar.semaforo, 0, 1);
